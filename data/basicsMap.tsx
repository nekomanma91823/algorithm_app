...
export const basicsMap: { [key: string]: BasicsContent } = {
  ...
  "gpu-architecture": {
    ...
    realWorldExample:
      "ゲームのグラフィックス処理、機械学習の学習・推論、仮想通貨のマイニング、科学技術計算、動画エンコーディング、3DCG制作など",
    importance:
  ...
  "memory-and-storage": {
    ...
    realWorldExample:
      "パソコンのRAMとSSD/HDD、スマートフォンの内蔵ストレージ、サーバーの大容量メモリシステム、データセンターのストレージアレイなど",
    importance:
  ...
  hardware: {
    ...
    realWorldExample:
      "デスクトップPC、ノートPC、スマートフォン、サーバー、ゲーム機、IoTデバイス、スーパーコンピュータなど",
    importance:
  ...
  software: {
    ...
    realWorldExample:
      "OS（Windows、macOS、Linux）、Webブラウザ、Word/Excel、ゲーム、スマートフォンアプリ、Webサイト、人工知能システムなど",
    importance:
  ...
  "process-thread": {
    ...
    realWorldExample:
      "Webブラウザの複数タブ（プロセス）、動画再生中の他作業（スレッド）、Webサーバーのリクエスト処理、ゲームの描画・音響・入力処理など",
    importance:
  ...
  "memory-management": {
    ...
    realWorldExample:
      "JavaやPythonの自動メモリ管理、C/C++の手動メモリ管理、OSの仮想メモリシステム、データベースのバッファプール管理など",
    importance:
  ...
  "logical-operations": {
    ...
    features:
      "AND、OR、NOT、XORなどの基本演算を組み合わせて複雑な条件判定や制御を行います。",
    example:
  ...
  "number-systems": {
    ...
    realWorldExample:
      "メモリアドレス（0x1000）、カラーコード（#FF0000）、ファイルの16進ダンプ、プロセッサの命令コード、ネットワークのMACアドレスなど",
    importance:
  ...
  "set-theory": {
    ...
    realWorldExample:
      "データベースのJOIN操作、検索エンジンのフィルタリング、プログラミングの配列操作、機械学習の特徴選択、アクセス権限の管理など",
    importance:
  ...
  complexity: {
    ...
    realWorldExample:
      "Googleの検索エンジン、データベースのインデックス、画像認識AI、ゲームの物理シミュレーション、暗号化アルゴリズム、リアルタイム通信システムなど",
    importance:
